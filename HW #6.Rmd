...
::::{.solution}
To sample uniformly from the set of directions in $\real^2$, we draw $U ~\unifdist[0,1]$, and return $(\cos(\tau U), \sin(\tau U))$.
...
hypersphere_6_mchain<-function(steps=10^5){
  ...
  x<-c(0,0,0,0,0,0)
  for(i in 1:burnin){
    x<-step_hypersphere_6(x, ms[,i]/sum(ms[,i]^2))
  }
  #output and datagathering
  output<-matrix(ncol=6, nrow=datasteps+1)
  output[1,]<-x
  ms<-replicate(burnin, rnorm(6, sigma))
  for(i in 1:datasteps){
    x<-step_hypersphere_6(x, ms[,i]/sum(ms[,i]^2))
    output[i+1,]<-x
  }
...
```
My estimate of the probability of a point being within 0.9 of the origin as being $\boxed{0.8146 \pm 0.0011}$.
::::
...
hypersphere_6_sigma<-function(steps=10^5, sigma=1, acceptance_assessment=TRUE){
  ...
  if(acceptance_assessment==TRUE){
    ...
    ms<-replicate(datasteps, rnorm(6, sigma))
    for(i in 1:datasteps){
      xold<-x
      x<-step_hypersphere_6(x, ms[,i]/sum(ms[,i]^2))
...
accept_vec<-replicate(10, mean(hypersphere_6_sigma(sigma=0.348)))
...
```
$\boxed{\sigma=0.348}$ produces a markov chain that accepts with probability $0.25405 \pm 0.00061$.
::::
